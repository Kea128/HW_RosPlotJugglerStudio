  ...
  : QDialog(parent), ui(new Ui::PreferencesDialog)
{
  ...
  ui->checkBoxAutomaticUpdates->setChecked(
      settings.value("Updates/automaticCheck", true).toBool());
  const QString update_channel =
      settings.value("Updates/channel", QStringLiteral("stable")).toString();
  ui->comboBoxUpdateChannel->setCurrentIndex(update_channel == QStringLiteral("beta") ? 1 : 0);
...
void PreferencesDialog::on_buttonBox_accepted()
{
  ...
  settings.setValue("Updates/automaticCheck", ui->checkBoxAutomaticUpdates->isChecked());
  settings.setValue("Updates/channel",
                    ui->comboBoxUpdateChannel->currentIndex() == 1 ? QStringLiteral("beta")
