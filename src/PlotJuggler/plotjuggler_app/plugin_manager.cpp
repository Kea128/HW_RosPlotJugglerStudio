...
void PluginManager::loadPluginsFromFolder(const QString& folderPath)
{
  ...
  for (const QString& filename : pluginsDir.entryList(QDir::Files))
  {
    ...
    QString canonicalPath = QFileInfo(pluginPath).canonicalFilePath();
    if (canonicalPath.isEmpty())
    {
      canonicalPath = QFileInfo(pluginPath).absoluteFilePath();
    }
    if (!_scanned_plugin_files.insert(canonicalPath).second)
    {
      continue;
    }
...
{
  auto plugin_loader = std::make_unique<QPluginLoader>(filename);
  if (plugin_loader->metaData().isEmpty())
  {
    // Portable Windows distributions keep runtime DLLs beside the executable.
    // Reject non-Qt libraries before loading them as plugin candidates.
    return;
  }
  ...
  if (!plugin && plugin_loader->errorString().contains("is not an ELF object") == false)
  {
    qDebug() << filename << ": " << plugin_loader->errorString();
    return;
  }
  if (plugin && dynamic_cast<PlotJugglerPlugin*>(plugin))
  {
    ...
    const auto isListed = [&](const QStringList& list) {
      return list.contains(plugin_name) || list.contains(fileinfo.baseName());
    };
    if (!_enabled_plugins.empty() && !isListed(_enabled_plugins))
    {
      qDebug() << message << " ...skipping, because it is not explicitly enabled";
      return;
    }
    if (!_disabled_plugins.empty() && isListed(_disabled_plugins))
    {
    ...
    if (_loaded_plugins.find(plugin_name) != _loaded_plugins.end())
    {
      qWarning() << message << " ...skipping duplicate plugin name:" << plugin_name;
      return;
    }
    qDebug() << message;
    ...
    if (loader)
    {
      _data_loader.emplace(plugin_name, DataLoaderPtr(loader, [](DataLoader*) {}));
    }
    else if (publisher)
    {
      _state_publisher.emplace(plugin_name,
                               StatePublisherPtr(publisher, [](StatePublisher*) {}));
    }
    else if (streamer)
    {
      _data_streamer.emplace(plugin_name, DataStreamerPtr(streamer, [](DataStreamer*) {}));
    }
    else if (message_parser)
    {
      QStringList encodings = QString(message_parser->encoding()).split(";");
      auto parser_ptr = ParserFactoryPtr(message_parser, [](ParserFactoryPlugin*) {});
    ...
    {
      _toolboxes.emplace(plugin_name, ToolboxPluginPtr(toolbox, [](ToolboxPlugin*) {}));
    }
    _plugin_loaders.push_back(std::move(plugin_loader));
  }
}
...
void PluginManager::unloadAllPlugins()
{
  ...
  for (auto it = _plugin_loaders.rbegin(); it != _plugin_loaders.rend(); ++it)
  {
    if (!(*it)->unload())
    {
      qWarning() << "Unable to unload plugin" << (*it)->fileName() << (*it)->errorString();
    }
  }
  _plugin_loaders.clear();
  _scanned_plugin_files.clear();
...
void PluginManager::loadWASM(const QString& pluginPath)
{
  ...
  try
  {
    ...
    if (_loaded_plugins.find(plugin_name) != _loaded_plugins.end())
    {
      qWarning() << "Skipping duplicate WASM plugin name:" << plugin_name;
      return;
    }
