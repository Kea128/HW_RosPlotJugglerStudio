...
ULogParser::ULogParser(DataStream& datastream) : _file_start_time(0)
{
  ...
  while (datastream && static_cast<int64_t>(datastream.offset) < _read_until_file_position)
  {
    ulog_message_header_s message_header;
    datastream.read((char*)&message_header, ULOG_MSG_HEADER_LEN);

    _read_buffer.resize(message_header.msg_size + 1);
    ...
    switch (message_header.msg_type)
    {
      ...
      case (int)ULogMessageType::REMOVE_LOGGED_MSG:
        if (message_header.msg_size < sizeof(uint16_t))
        {
          break;
        }
      ...
      case (int)ULogMessageType::LOGGING: {
        if (message_header.msg_size < sizeof(char) + sizeof(uint64_t))
        {
          break;
        }
        MessageLog msg;
...
void ULogParser::parseDataMessage(const ULogParser::Subscription& sub, char* message)
{
  if (!sub.format)
  {
    qWarning() << "ULog: skipping data message without a known format";
    return;
  }
...
{
  _read_buffer.resize(msg_size + 1);
  char* message = (char*)_read_buffer.data();
...
bool ULogParser::readInfo(DataStream& datastream, uint16_t msg_size)
{
  ...
  std::string value;
  auto readValue = [&raw_value](auto& result) {
    if (raw_value.size() < sizeof(result))
    {
      return false;
    }
    memcpy(&result, raw_value.data(), sizeof(result));
    return true;
  };
