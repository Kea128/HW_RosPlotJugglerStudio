...
QDomElement saveChildNodesState(QDomDocument& doc, QWidget* widget)
{
  ...
  if (splitter)
  {
    ...
    {
      const double ratio =
          total_size > 0 ? double(size) / double(total_size) : 1.0 / splitter->count();
      sizes_str += QString::number(ratio);
      sizes_str += ";";
    }
...
void PlotDocker::restoreSplitter(QDomElement elem, DockWidget* widget)
{
  ...
  if (!widget || splitter_count < 1 ||
      (!orientation_str.startsWith("|") && !orientation_str.startsWith("-")))
  {
    return;
  }
  ...
  for (int i = 0; i < splitter_count; i++)
  {
    ...
    const double ratio =
        i < sizes_str.size() ? sizes_str[i].toDouble(&valid_ratio) : 0.0;
    sizes.push_back(valid_ratio && ratio > 0.0
                        ? static_cast<int>(ratio * std::max(tot_size, splitter_count))
                        : 1);
  }
  ...
  {
    if (index >= static_cast<int>(widgets.size()))
    {
      qWarning() << "Ignoring extra child in saved plot splitter layout";
      break;
    }
...
DockWidget* DockWidget::splitHorizontal()
{
  ...
  if (auto splitter = ads::internal::findParent<ads::CDockSplitter*>(new_widget))
  {
    QList<int> equal_sizes;
    for (int index = 0; index < splitter->count(); ++index)
    {
      equal_sizes.push_back(1);
    }
