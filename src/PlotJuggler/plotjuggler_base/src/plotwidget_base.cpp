...
Range PlotWidgetBase::getVisualizationRangeY(Range range_X) const
{
  ...
  {
    // Keep constant-valued signals centered and visible instead of relying on
    // Qwt's arbitrary fallback range for identical axis limits.
    margin = std::max(std::abs(top) * 0.025, 1.0);
  }
...
PlotWidgetBase::PlotWidgetBase(QWidget* parent) : _xy_mode(false), _keep_aspect_ratio(false)
{
  ...
  QSettings settings;
  // The OpenGL canvas can leave the entire top-level window blank on some
  // Windows GPU/remote-desktop combinations. Keep the raster canvas as the
  // safe default; users may explicitly enable OpenGL in Preferences.
