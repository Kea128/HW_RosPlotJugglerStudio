...
#include "studio/update/update_coordinator.h"
  ...
  , _update_coordinator(nullptr)
{
  ...
  initializeActions();
  _update_coordinator = new StudioUpdate::UpdateCoordinator(this);
  connect(ui->actionCheckForUpdates, &QAction::triggered, _update_coordinator,
          &StudioUpdate::UpdateCoordinator::checkManually);
  connect(_update_coordinator, &StudioUpdate::UpdateCoordinator::normalCloseRequested, this,
          &MainWindow::close, Qt::QueuedConnection);
  QTimer::singleShot(2000, _update_coordinator,
                     &StudioUpdate::UpdateCoordinator::checkAutomatically);
...
void MainWindow::loadAllPlugins(QStringList command_line_plugin_folders)
{
  ...
  builtin_folders += application_dir;
  const QString configured_plugin_dir = QStringLiteral(PJ_PLUGIN_INSTALL_DIRECTORY);
  builtin_folders += QDir::isRelativePath(configured_plugin_dir)
                         ? QDir(application_dir).filePath(configured_plugin_dir)
...
void MainWindow::on_actionAbout_triggered()
{
  ...
  ui->label_version->setText(QStringLiteral("version: ") + QStringLiteral(PJ_STUDIO_VERSION));
